/**
 * @file GenericDynamicArraySerialization.c
...
int serializeDynamicArray(DynamicArray* da, const char* filename)
{
    ...
    if ((fwrite(&da->capacity, sizeof(da->capacity), 1, file) != 1) ||
        ...
        (fwrite(&da->elementSize, sizeof(da->elementSize), 1, file) != 1)) 
    {
    ...
    if (da->size > 0) 
    {
        if (fwrite(da->array, da->elementSize, da->size, file) != da->size) {
...
DynamicArray* deserializeDynamicArray(const char* filename, size_t elementSize, CopyFunction copyFunc, DestroyFunction destroyFunc, CompareFunction compFunc)
{
    ...
    if (fread(&result->capacity, sizeof(result->capacity), 1, file) != 1 ||
        ...
        fread(&result->elementSize, sizeof(result->elementSize), 1, file) != 1) {
    ...
    result->array = malloc(result->capacity * result->elementSize);
    ...
    if (result->size > 0) 
    {
        if (fread(result->array, result->elementSize, result->size, file) != result->size) {
